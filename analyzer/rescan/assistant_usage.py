"""analyzer/rescan/assistant_usage.py — transcript から token / model 観測を
集めて `assistant_usage` event を導出する共通コア (Issue #99)。

live Stop hook (`hooks/record_assistant_usage.py`) と rescan
(`analyzer/rescan/transcripts.py`) の両方から利用される。
...
from analyzer.hot_append import append_event
