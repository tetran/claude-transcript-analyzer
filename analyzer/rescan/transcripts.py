"""analyzer/rescan/transcripts.py — 過去トランスクリプト遡及スキャンの共通コア。

~/.claude/projects/ 以下の過去のトランスクリプト（.jsonl）を遡って解析し、
usage.jsonl を一から作り直す（または追記する）ためのロジック。

Claude Code Hooks は導入後のイベントしか記録できないため、
このコアで過去の使用履歴を遡及して収集できる。CLI 起動口は
...
from analyzer.rescan.assistant_usage import (
