...
並列耐性:
- EX を取得して実行 (record_*.py 側は **blocking SH** で release を待つ /
  Phase A1 + codex 5th P1 で旧 NB×5 retry の data loss 経路を撤廃)
- lock 層は `hooks/_lock.py` 経由で POSIX (`fcntl.flock`) / Windows
  (`msvcrt.locking`) の差を吸収 (Issue #44)
