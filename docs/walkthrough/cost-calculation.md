...
- `model` — assistant message を生成したモデル ID。**`/model` 切替があれば session 内でも値が変わる**のでメッセージ単位で記録する必要がある
- `input_tokens` / `output_tokens` / `cache_read_tokens` / `cache_creation_tokens` — Anthropic の課金 4 軸。transcript 上は `cache_read_input_tokens` / `cache_creation_input_tokens` だが、hookでは `_input_tokens` サフィックスを `_tokens` に統一して保存している (`docs/spec/usage-jsonl-events.md` § assistant_usage の対応表)
- `message_id` — `(session_id, message_id)` のペアで重複は先勝ちで保存。rescan 二重実行 / hook 再発火 / main + subagent 二重観測でも 1 件に集約する冪等性保証のためのキー
- `service_tier` — `priority` / `standard` 等の値をそのまま使用。欠損時は `null`。Sessions ページの `service_tier_breakdown` の入力
- `source` — `"main"` (メイン session transcript) / `"subagent"` (`<session>/subagents/agent-*.jsonl`) の 2 値。Issue #93 の `subagent_type == ""` フィルタを通った type あり呼び出しのみ subagent 経路に入る。
...
- 単位は **USD / 1M tokens**。`(tokens / 1_000_000) * rate` の素直な式に揃えて、メンタルモデルを単純化している。
- 4 桁丸め (USD 0.0001 = 1/100 セント) で表示誤差を最小化。集計後の合算では十分な精度と考えているため。
- 上の例では cost の **96% が `cache_creation` 成分** (0.219 / 0.227)。Claude Code の典型的な workload ではキャッシュ系がコストを支配しがちなので、cache系含めて保存が必要。
...
3. **Sonnet fallback**: どれも hit しなければ `claude-sonnet-4-6` として扱う。
...
ここで重要なのは **「イベントごとに rate を当ててから sum する」** こと。`SUM(input_tokens)` を取ってから 1 つの rate を掛ける素朴な設計だと、session 内で `/model haiku` 等で切り替わった場合に opus と haiku の token を一緒くたにしてしまい数字が壊れます (`docs/reference/cost-calculation-design.md` §5)。
...
`demo-app` session は同じ token 構成 (in=10 / out=1500、cache の方向が opus 側は creation 中心、haiku 側は read 中心) で `/model` だけ切り替わっています。1 件目 (opus) が $0.1626、2 件目 (haiku) が $0.0095 で **約 17 倍** の差。これがメッセージ単位での集計を採用することで初めて見える節約効果です。
...
3. `assistant_usage` を全件走査して `models` (モデル別 message 数) / 4 軸 token 累計 / `service_tier_breakdown` を作る
4. 同じループでメッセージごとのコストを `calculate_message_cost` で算出して `cost_total` に積む
5. `skill_count` (`skill_tool` + `user_slash_command`) と `subagent_count` (`session_subagent_counts`) を足して `rows` に
6. `started_at` 降順でソートし、先頭 `TOP_N_SESSIONS = 20` 件を返す

fixture 全体を流し込んで `rows` を見てみます。
...
- `models` は session 内 `/model` 切替もメッセージ単位で計算される (demo-app session が opus + haiku の両 key を持つ)
- `service_tier_breakdown` は **欠損 / null を除外**、値が存在する場合のみ集計
- `ended_at` / `duration_seconds` は `session_end` 不在で両方とも `null` (= UI 側「進行中」pill の表示条件)
- 並び順は `started_at` 降順 (最新 session が先頭)。10:00 開始の demo-app session が、08:25 開始の gh-extensions session より先に来ている

`aggregate_session_breakdown(events, period_events=...)` へ引数を2つ渡しているのは表示期間（7d/30d/90d）を跨ぐ session を適切に表示させるための工夫です: `session_start` / `session_end` は全期間のイベントから、content (= cost / token / models / service_tier / skill_count) は選択期間内のイベントから、別々に取ります。これで `session_start` が期間開始日より前にあっても、期間内に `assistant_usage` がある session は cost / token を期間内限定で合算した状態で表示されます。詳しい契約は `docs/spec/dashboard-api.md` §session_breakdown 参照。
...
- 💡 **採用判断の研究ノート (なぜ DB に直接コストを保存しないか / なぜメッセージごとの集計か)** → `docs/reference/cost-calculation-design.md`
