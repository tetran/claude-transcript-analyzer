...
## §5. `dashboard/` パッケージレイアウト — server.py の責務分割 (Issue #123)

`dashboard/server.py` は 1721 行まで肥大化していたため、`#123` で責務単位の 5 サブモジュールへ内部分割した。`dashboard/` を `__init__.py` 付きパッケージへ昇格し、`server.py` 自身は **re-export shim 兼エントリポイント** として残す。

```
dashboard/
├── __init__.py       # パッケージマーカー (空)
├── server.py         # re-export shim + `if __name__ == "__main__"` エントリ
├── config.py         # env 解決 + モジュール定数 (DATA_FILE / PORT / _PERIOD_DELTAS ...)
├── aggregate.py      # period filter + 全 aggregate_* + load_events / load_health_alerts
├── api.py            # build_dashboard_data (aggregate + analyzer を束ねる集計グルー)
├── render.py         # render_static_html + _build_html_template + _HTML_TEMPLATE
└── http_runtime.py   # SSEClient / _FileWatcher / _IdleTracker / DashboardHandler /
                      #   DashboardServer / create_server / run / main
```
...
`config.py` と `render.py` はリーフ。`api.py` と `render.py` は兄弟でエッジを持たない (`render_static_html` は `data: dict` を受けるだけで `build_dashboard_data` を呼ばない)。`http_runtime.py` が両者に依存する。

### 非自明ポイント

- **`render.py` という命名**: 区画 D は元々「テンプレート組み立て」だが、`dashboard/template.py` は既存の `dashboard/template/` ディレクトリと Python モジュール名が衝突する。衝突回避のため `render.py` とした。
- **shim がサブモジュールを reload する理由**: `tests/test_dashboard.py` は `server.py` を `spec_from_file_location` で **ファイルパス直 import** し、その都度 `USAGE_JSONL` 等の env をパッチして fresh module として exec する。サブモジュール (`dashboard.config` 等) は通常のパッケージ import で `sys.modules` にキャッシュされ 2 回目以降 re-exec されないため、shim は exec のたびに依存順 (config → aggregate → render → api → http_runtime) で `importlib.reload` を回し、import 時 env 評価 (`config.DATA_FILE` / `render._HTML_TEMPLATE`) を patch 済み env 下で再評価させる。reload しないと env override が効かずテスト隔離が静かに壊れる。
- **`sys.path.insert` は shim の最初の実行行**: shim は file 直 import されるため、`import dashboard.config` 等のサブパッケージ import を解決するには repo root を `sys.path` に積む必要がある。サブモジュール側は `sys.path` ハックを持たない (shim が先に整える)。
- **外部 importer は無改修**: `reports/export_html.py` の `from dashboard.server import build_dashboard_data, render_static_html` 等、従来の import パスは shim の re-export でそのまま生きる。新パス (`dashboard.aggregate` 等) への移行は別 Issue。
