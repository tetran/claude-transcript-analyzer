...
(`analyzer/subagent.py` / `dashboard/server.py:aggregate_hourly_heatmap` /
将来の `reports/summary.py`...)。**naive datetime (tz 無し) の扱いを
全 aggregator で揃えないと、同じ入力に対して divergent 出力** を出す。

### 観測されたミスマッチ

- `analyzer.subagent._week_start_iso`: naive を **UTC として扱う** (`replace(tzinfo=UTC)`)
...
2. aggregator docstring に policy + back-pointer (例:
   "naive datetime は UTC として扱う (`analyzer.subagent._week_start_iso`
   と統一)")
...
- naive 扱いの実装: `analyzer/subagent.py:_week_start_iso()`
...
| 概念 | 実装場所 |
|---|---|
| `usage.jsonl` 一次書込 | `hooks/record_*.py` 各種（lock 付き append は `analyzer/hot_append.py` / `analyzer/platform/lock.py` 経由） |
| Hot tier ファイルパス解決 | `~/.claude/transcript-analyzer/usage.jsonl`（実体）/ `USAGE_JSONL` env で override |
| Archive merge / dedup | **実装済** (Issue #30, v0.6.0〜) — `analyzer/archive/usage.py:_structural_fingerprint()` で tier 1/2/3 fingerprint dispatch。詳細仕様は `docs/transcript-format.md` の "Archive 互換性のための schema 進化規約" を参照 |
