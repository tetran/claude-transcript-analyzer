...
| **モデル別集約 → 合計** | 中 | `analyzer/subagent.py` 隣に `analyzer/cost.py` 新設。集約 dict を model 別に持ち、reduce 適用 |
...
| ✅ 採用 | §2 4 種 token × per-1M rate | `analyzer.cost.MODEL_PRICING` に dict で持つ (Python 実装) |
...
- Issue #99: infra (`assistant_usage` event + `analyzer.cost` + main / per-subagent transcript 収集 + `/api/data` 拡張)
...
- 集計ロジックの宿先: `analyzer/subagent.py` と並ぶ位置に `analyzer/cost.py` を置く想定
