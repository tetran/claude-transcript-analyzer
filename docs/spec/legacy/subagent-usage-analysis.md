# Subagent (Task/Agent ツール) 使用状況の調べ方
...
ツール名は **`Task`** または **`Agent`**（Claude Code のバージョンによって異なる）。
...
| フィールド | 内容 |
|-----------|------|
| `subagent_type` | エージェント種別（`Explore`, `Plan`, `general-purpose`, `ui-designer` 等） |
| `description` | 短い説明（3〜5語） |
| `prompt` | エージェントへの詳細な指示 |
| `run_in_background` | バックグラウンド実行かどうか（`true`/`false`、`Task` のみ） |

> **注意**: `SubagentStart` フックで受け取るデータは `subagent_type` ではなく **`agent_type`** フィールドに入る。
...
```bash
grep -rE '"name":"(Task|Agent)"' ~/.claude/projects/ --include="*.jsonl" -h \
