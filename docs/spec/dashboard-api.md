...
- 三段 pair-straddling filter: timestamp 第一段 cut 後、`subagent_start ↔ subagent_lifecycle_start` の
  ...
  `analyzer.subagent._pair_invocations_with_stops` の `start_ts <= stop_ts < next_start_ts`
...
- データ源: `analyzer.subagent.invocation_records()` 経由で `_build_invocations` +
...
for each notification N (permission|permission_prompt) in session S:
  ...
  for ev in usage_invocation_events(S):
    start_ts, end_ts = analyzer.subagent.subagent_invocation_interval(ev)
...
- `estimated_cost_usd`: model 別 rate 適用 → reduce 合算 (=
  `analyzer.cost.calculate_session_cost()`)。**実測 token × 価格表掛け算による
...
- `subagent_count`: `analyzer.subagent.session_subagent_counts(events)` 経由
...
- top-N: `TOP_N_SESSIONS = 20` (`analyzer/cost.py` 定数)
- `TOP_N` (`dashboard/server.py`、ranking 系の 10) とは別の独立定数

### 価格表 / 未知 model fallback

- 価格表 (per-1M-token rate) は `analyzer.cost.MODEL_PRICING` に pin
...
フィールドを **family rollup (fable / opus / sonnet / haiku)** で集計した
messages × cost の二軸 distribution を返す。
...
```json
{
  "model_distribution": {
    "families": [
      { "family": "fable",  "messages": 24,  "messages_pct": 0.04, "cost_usd": 9.8765,  "cost_pct": 0.13 },
      { "family": "opus",   "messages": 312, "messages_pct": 0.58, "cost_usd": 48.5012, "cost_pct": 0.64 },
      { "family": "sonnet", "messages": 175, "messages_pct": 0.33, "cost_usd": 14.0123, "cost_pct": 0.18 },
...
- filter: `event_type == "assistant_usage"` のみ。`session_start` / `skill_tool` /
  `subagent_start` 等は除外
- family rollup: 各 event の `model` を `analyzer.cost.infer_model_family()` で
  family 文字列に解決 (substring match `fable` → `opus` → `haiku` → `sonnet` の優先順、
  未知 model は sonnet fallback)。client 側の `inferModelFamily`
  (`45_renderers_sessions.js`) と semantics を 1:1 一致
- per-event cost: `analyzer.cost.calculate_message_cost(model, in, out, cr, cc)`
...
`families` 配列の順は **常に `fable → opus → sonnet → haiku` の固定順**
(価格帯降順、Issue #128)。cost 降順や出現順ではない。理由:
...
family 数が 0〜3 でも `families` は **必ず 4 行** (未出現 family は
`messages=0` / `cost_usd=0` のゼロ行で埋める)。完全空 events のときも 4 行
全 zero で返す。
