...
from analyzer.platform.process import is_pid_alive, spawn_detached
...
def _remove_stale_server_json() -> bool:
    ...
    `_file_lock` + compare-and-delete を経由する。事前 `is_pid_alive` 判定後に
...
def _spawn_server() -> Optional[subprocess.Popen]:
    ...
    OS 別 detach 経路は `analyzer.platform.process.spawn_detached` に集約済み
    (Issue #121): POSIX は `start_new_session`、Windows は `creationflags`。
    """
    if not _SERVER_SCRIPT.exists():
        return None
    return spawn_detached([sys.executable, str(_SERVER_SCRIPT)])
